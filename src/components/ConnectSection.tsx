...
export default function ConnectSection() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      // Kinetic header: characters rotate in from rotateX(-90deg)
      if (headerRef.current) {
        const chars = headerRef.current.querySelectorAll('.split-char');
        gsap.set(chars, { opacity: 0, rotateX: -90, transformPerspective: 800 });
      ...
      if (backgroundOrbsRef.current) {
        ...
        gsap.set(orbs, { opacity: 0, scale: 0.3 });
        ...
        orbs.forEach((orb, index) => {
          gsap.to(orb, { y: -30, duration: 4 + index * 0.5, ease: "power2.inOut", yoyo: true, repeat: -1 });
          gsap.to(orb, { x: 20, duration: 5 + index * 0.3, ease: "power2.inOut", yoyo: true, repeat: -1, delay: index * 0.3 });
        });
      }

      // Contact cards
      if (contactMethodsRef.current) {
        const cards = contactMethodsRef.current.querySelectorAll('.contact-card');
        gsap.fromTo(cards,
          { opacity: 0, y: 80, rotateY: -20, scale: 0.8 },
          {
            opacity: 1, y: 0, rotateY: 0, scale: 1,
            duration: 1, stagger: 0.2, ease: "back.out(1.7)",
            scrollTrigger: { trigger: contactMethodsRef.current, start: "top 80%", once: true }
          }
        );
      }
      ...
      gsap.fromTo(ctaButtonRef.current,
        { opacity: 0, y: 60, scale: 0.7 },
        {
          ...
          scrollTrigger: { trigger: ctaButtonRef.current, start: "top 85%", once: true }
        }
      );
      ...
      if (socialLinksRef.current) {
        ...
        gsap.fromTo(icons,
          { opacity: 0, y: 20, scale: 0.8 },
          {
            ...
            scrollTrigger: { trigger: socialLinksRef.current, start: "top 90%", once: true }
          }
        );
      }
  ...
  return (
    ...
    >
      ...
      <div ref={backgroundOrbsRef} className="absolute inset-0">
        <div className="floating-orb absolute top-1/4 left-1/4 w-96 h-96 bg-gradient-to-r from-purple-500/10 to-pink-500/10 rounded-full blur-3xl"></div>
        <div className="floating-orb absolute bottom-1/4 right-1/4 w-80 h-80 bg-gradient-to-r from-cyan-500/10 to-blue-500/10 rounded-full blur-3xl"></div>
        <div className="floating-orb absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-gradient-to-r from-green-500/10 to-teal-500/10 rounded-full blur-3xl"></div>
      </div>

      <div className="relative z-10 max-w-6xl mx-auto text-center">
        {/* Header */}
        <div className="mb-16">
          <h2 ref={headerRef} className="text-4xl md:text-6xl lg:text-7xl font-bold mb-6" aria-label="Let's Connect">
            {splitTitle("Let's Connect", "Connect")}
          </h2>
          <p ref={descRef} className="text-xl md:text-2xl text-gray-300 max-w-4xl mx-auto leading-relaxed mb-8">
        ...
        <div ref={contactMethodsRef} className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
          {connectData.contactMethods.map((method, index) => (
            ...
            >
              <TiltCard className="bg-gray-900/60 backdrop-blur-sm border border-gray-700 rounded-2xl p-8 h-48 flex flex-col items-center justify-center text-center hover:border-purple-400/50 hover:shadow-lg hover:shadow-purple-400/20 transition-all duration-300 group">
        ...
        <div ref={footerRef} className="text-center">
          ...
          <p className="text-gray-500 text-sm">{connectData.footer}</p>
        </div>
      </div>
    </section>
  );
