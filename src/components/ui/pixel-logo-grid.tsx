...
function LogoCard({ name, logo: Logo, pixelColors, className }: LogoCardProps) {
  return (
    <div
      className={cn(
        "group relative flex items-center justify-center rounded-xl border border-white/10 bg-black/50 p-8 backdrop-blur-sm transition-colors hover:border-white/20",
        className
      )}
    >
      <PixelCanvas colors={pixelColors} gap={6} speed={25} />
      <Logo
        className="relative z-10 h-6 w-auto text-white/60 transition-all duration-300 group-hover:text-white"
        style={{}}
      />
...
export { PixelCanvas, LogoCard };
export type { PixelCanvasProps, LogoCardProps, LogoSvgProps };
